...
function SettingsPage() {
  ...
  async function handleDeleteHousehold() {
    ...
    if (
      !confirm(
        "UWAGA! Czy na pewno chcesz USUNĄĆ całe gospodarstwo? Ta operacja jest nieodwracalna i usunie dane WSZYSTKICH członków.",
      )
    ) {
      return;
    }
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-8 animate-fade-up">
      <header className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-3 leading-relaxed max-w-2xl">
            Skonfiguruj swoje gospodarstwo domowe, zaproś partnera i dostosuj parametry podatkowe do
      ...
      <Tabs defaultValue="household" className="space-y-8">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Synchronizowane między domownikami
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  {editingName ? (
                    <div className="flex gap-2 w-full">
                      <Input
                        ...
                        autoFocus
                      />
                      <Button size="sm" onClick={handleSaveHouseholdName} disabled={loading}>
                        Zapisz
                      </Button>
                    ...
                    <>
                      ...
                      {isOwner && (
                        <button
                          ...
                          className="p-1.5 hover:bg-background rounded-lg transition-colors text-muted-foreground"
                        >
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {members.map((m) => (
                      <div
                        key={m.user_id}
                        className="flex items-center justify-between p-4 bg-muted/20 rounded-2xl border border-border/40 hover:bg-muted/40 transition-colors"
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {m.role === "owner" && (
                                <Badge
                                  variant="outline"
                                  className="bg-accent/10 text-accent border-accent/20 text-[9px] px-1.5 h-4"
                                >
                                  Właściciel
                                </Badge>
                              )}
                        ...
                        {isOwner && m.user_id !== session?.user.id && (
                          <div className="flex gap-2">
                            <Button
                              ...
                              onClick={() => {
                                if (confirm(`Przekazać własność do ${m.nickname || m.email}?`))
                                  transferOwnership(getActiveHouseholdId()!, m.user_id).then(() =>
                                    refreshHouseholdInfo(),
                                  );
                              }}
                            >
                              Przekaż
                            ...
                            <Button
                              ...
                              onClick={() => {
                                if (confirm(`Usunąć ${m.nickname || m.email} z gospodarstwa?`))
                                  removeHouseholdMember(getActiveHouseholdId()!, m.user_id).then(
                                    () => refreshHouseholdInfo(),
                                  );
                              }}
                            >
              ...
              <div className="bg-card rounded-[2rem] border border-border p-8 shadow-sm space-y-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Partner lub członek rodziny będzie widział te same dane co Ty.
                  </p>
                </div>
                  ...
                  <div className="pt-6 border-t border-border/50">
                    ...
                    <div className="space-y-2">
                      {pendingInvites.map((inv) => (
                        <div
                          key={inv.id}
                          className="flex items-center justify-between p-3 bg-muted/10 rounded-xl border border-border/30"
                        >
                          <span className="text-xs font-medium">{inv.email}</span>
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] text-muted-foreground">
                              Wygasa {new Date(inv.expires_at).toLocaleDateString()}
                            </span>
                            {isOwner && (
                              <button
                                onClick={() =>
                                  revokeHouseholdInvite(inv.id).then(() => refreshHouseholdInfo())
                                }
              ...
              <div className="bg-accent rounded-[2rem] p-8 text-accent-foreground shadow-warm relative overflow-hidden group">
                ...
                <div className="space-y-4 relative z-10">
                  ...
                  <p className="text-[10px] leading-relaxed opacity-60 italic">
                    Wszystkie dane są szyfrowane i synchronizowane w czasie rzeczywistym między
                    Twoimi urządzeniami oraz zaproszonymi domownikami.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/50 pb-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Globalne wartości używane do wszystkich kalkulacji.
                </p>
              </div>
              <Button
                variant="outline"
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                    Rok regulacyjny (IKE/IKZE)
                  </Label>
                  <Input
                    type="number"
                    min={2020}
                    max={2100}
                    value={globalSettings.regulatoryYear}
                    onChange={(e) =>
                      actions.updateGlobalSettings({
                        regulatoryYear: Number(e.target.value) || 2025,
                      })
                    }
                    className="font-mono"
                  />
                  <p className="text-[10px] text-muted-foreground italic">
                    Uzywany do doboru limitow IKE/IKZE.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-muted-foreground italic">
                    Podstawa do wyliczania limitu 30-krotności składek ZUS.
                  </p>
                </div>

                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                    Próg II skali podatkowej (rocznie)
                  </Label>
                  <div className="relative">
                    <Input
                      type="text"
                ...
                <div className="space-y-3 bg-accent/5 p-6 rounded-2xl border border-accent/10">
                  ...
                  <div className="flex items-center gap-4">
                    <Input
                      ...
                      onChange={(e) =>
                        actions.updateGlobalSettings({
                          targetEmergencyFundMonths: Number(e.target.value),
                        })
                      }
                  ...
                  <p className="text-[10px] text-muted-foreground leading-relaxed mt-2 italic">
                    To ustawienie wpłynie na wskaźnik "Poduszka" na pulpicie, pokazując czy Twój
                    zapas gotówki jest wystarczający względem Twojego osobistego celu.
                  </p>
                </div>
              </div>
            </div>
        ...
        <TabsContent
          value="profile"
          className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-2xl"
        >
          <div className="bg-card rounded-[2rem] border border-border p-8 shadow-warm space-y-8">
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-muted-foreground">
                  To imię będzie wyświetlane w powitaniu na pulpicie oraz przy Twoich zmianach w
                  historii.
                </p>
              </div>
            ...
            <div className="bg-card rounded-[2rem] border border-border p-8 shadow-warm space-y-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Pobierz kopię zapasową całego gospodarstwa w formacie JSON. Możesz ją wykorzystać
                  do własnych analiz lub jako kopię bezpieczeństwa.
                </p>
              </div>
              <Button
                variant="outline"
              ...
              <div className="space-y-3">
                <Button
                  ...
                  onClick={() => {
                    if (
                      confirm(
                        "Czy na pewno chcesz wyczyścić wszystkie dane? Ta operacja usunie zarobki, wydatki i aktywa, ale zachowa członków gospodarstwa.",
                      )
                    ) {
