...
import { Plus, RotateCcw, Users, Zap, Wallet, Info, ArrowRight } from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 space-y-12">
      ...
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="space-y-4">
          ...
          <h1 className="font-display text-4xl sm:text-6xl font-semibold tracking-tight">
            Twoje <span className="italic text-accent decoration-accent/30 underline underline-offset-8">wynagrodzenie</span>
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-4">
          {spouses.length >= 2 && (
            <div className="flex items-center gap-4 bg-card border border-border rounded-2xl px-5 py-3 shadow-sm hover:shadow-md transition-shadow">
               <div className="flex flex-col">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground leading-none mb-1">Rozliczenie wspólne</span>
          ...
          <div className="flex items-center gap-2">
            <Button size="lg" onClick={() => actions.addSpouse()} className="rounded-2xl px-6 h-14 shadow-lg shadow-primary/20">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent className="rounded-[2rem]">
                <AlertDialogTitle className="font-display text-2xl">Zresetować dane?</AlertDialogTitle>
                <AlertDialogDescription className="text-base">
                  Spowoduje to wyczyszczenie wszystkich informacji o wynagrodzeniach, wydatkach i aktywach. 
                  Tej operacji nie można cofnąć.
                </AlertDialogDescription>
                <AlertDialogFooter className="gap-3">
                  <AlertDialogCancel className="rounded-xl">Anuluj</AlertDialogCancel>
                  <AlertDialogAction onClick={() => actions.reset()} className="bg-destructive text-destructive-foreground hover:bg-destructive/90 rounded-xl">
                    Wyczyść wszystko
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
          ...
          <div className="xl:col-span-2 bg-card rounded-[3rem] p-16 border border-dashed border-border/60 flex flex-col items-center text-center space-y-6 animate-in fade-in duration-700">
            ...
            <div className="max-w-md space-y-2">
              ...
              <p className="text-muted-foreground text-lg leading-relaxed">
                Dodaj siebie lub domowników, aby obliczyć wspólny budżet i zobaczyć prognozy na 2025 rok.
              </p>
            </div>
            <Button size="xl" onClick={() => actions.addSpouse()} className="rounded-3xl shadow-xl shadow-primary/20">
           ...
           <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-10">
              ...
              {jointFiling && joint && joint.savings > 0 && (
                <div className="bg-success-soft/30 backdrop-blur-sm rounded-[2rem] p-8 border border-success/20 flex flex-col items-center md:items-end text-center md:text-right min-w-[280px]">
                   ...
                   <p className="text-xs text-success/70 font-bold uppercase tracking-tighter italic">Roczny zysk z rozliczenia wspólnie</p>
                </div>
              )}
           </div>
        </div>
      ...
      <footer className="pt-10 border-t border-border/50 flex flex-col sm:flex-row items-center justify-between gap-6 text-muted-foreground text-xs font-medium">
         ...
         <div className="flex items-center gap-2 hover:text-accent transition-colors cursor-pointer group">
            Zobacz szczegółową analizę podatkową <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
         </div>
      </footer>
    </div>
  );
