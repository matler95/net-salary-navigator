...
import { LogOut, UserPlus, Users, ArrowRight, ShieldCheck, UserCircle, RefreshCcw } from "lucide-react";
import { getSupabase } from "@/lib/supabase";
import { useAuthSession } from "@/lib/auth";
import { acceptInvite, PENDING_INVITE_TOKEN_KEY, clearAppState, ACTIVE_HOUSEHOLD_KEY } from "@/lib/store";
  ...
  if (!token) {
    return (
      <main className="max-w-2xl mx-auto px-4 py-24 text-center space-y-6">
        ...
        <p className="text-muted-foreground max-w-sm mx-auto">Ten link jest nieprawidłowy lub nie zawiera wymaganych danych.</p>
        <Button asChild className="rounded-full px-8"><Link to="/">Wróć do aplikacji</Link></Button>
      </main>
    );
  }
      ...
      <div className="max-w-4xl w-full grid lg:grid-cols-2 gap-8 lg:gap-0 bg-card rounded-[2.5rem] border border-border shadow-[var(--shadow-elevated)] overflow-hidden animate-fade-up">
        ...
        <div className="bg-accent p-8 sm:p-12 text-accent-foreground flex flex-col justify-between relative overflow-hidden">
           ...
           <div className="relative z-10 space-y-6">
              <div className="w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center font-display font-bold italic text-xl">S</div>
              ...
              <p className="text-lg opacity-80 leading-relaxed max-w-sm">
                Zostałeś zaproszony do wspólnego zarządzania budżetem w gospodarstwie <span className="font-bold underline decoration-white/40">{householdName}</span>.
              </p>
           </div>
           ...
           <div className="relative z-10 pt-12">
              <div className="flex items-center gap-4 bg-black/10 backdrop-blur-sm p-4 rounded-2xl border border-white/10">
                 ...
                 <div className="min-w-0">
                    <p className="text-[10px] uppercase tracking-wider opacity-60 font-bold">Zaproszenie dla</p>
             ...
             <div className="space-y-8">
                {status && (
                  <div className={cn(
                    ...
                    status.type === "error" ? "bg-destructive/5 border-destructive/20 text-destructive" : "bg-success/5 border-success/20 text-success"
                  )}>
                    <p>{status.msg}</p>
                  </div>
                )}
                ...
                {wrongAccount ? (
                  <div className="space-y-6">
                    <div className="space-y-2">
                       ...
                       <p className="text-sm text-muted-foreground leading-relaxed">
                          Jesteś obecnie zalogowany jako <span className="font-bold text-foreground">{session?.user.email}</span>. 
                          To zaproszenie jest przeznaczone dla kogoś innego.
                       </p>
                    </div>
                    <div className="flex flex-col gap-3">
                       <Button onClick={handleSwitchAccount} variant="outline" className="w-full rounded-full h-12 gap-2">
                  ...
                  <div className="space-y-8">
                     <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">Potwierdź swoją ksywkę i dołącz do domowników.</p>
                     </div>
                     <div className="space-y-6">
                        <div className="space-y-2">
                           <Label className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground ml-1">Twoja ksywka</Label>
                           <Input 
                             value={nickname}
                        ...
                        <Button onClick={handleAccept} disabled={accepting} className="w-full h-12 rounded-full gap-2 text-base shadow-warm">
                           {accepting ? <RefreshCcw className="w-4 h-4 animate-spin" /> : <Users className="w-4 h-4" />}
                  ...
                  <div className="space-y-8">
                     ...
                     <form onSubmit={handleRegister} className="space-y-6">
                        <div className="space-y-4">
                           <div className="space-y-2">
                              <Label className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground ml-1">Ksywka</Label>
                              <Input 
                                value={nickname}
                                onChange={e => setNickname(e.target.value)}
                           ...
                           <div className="space-y-2">
                              <Label className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground ml-1">Hasło</Label>
                              <Input 
                                type="password"
                                value={password}
                        ...
                        <Button type="submit" disabled={registering || !password.trim()} className="w-full h-12 rounded-full gap-2 text-base shadow-warm">
                           {registering ? <RefreshCcw className="w-4 h-4 animate-spin" /> : <UserPlus className="w-4 h-4" />}
