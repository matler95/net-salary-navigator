      ...
      GET: async ({ request }) => {
        try {
          ...
          const [{ data: existingBonds }, { data: existingIndicators }] = await Promise.all([
            supabase.from('bond_data').select('*').eq('is_active', true).order('fetched_at', { ascending: false }),
          ...
          const needsRefresh = forceRefresh || isStale || !existingBonds || existingBonds.length === 0;
          ...
          const nbpIndicator = latestIndicators?.find((i: any) => i.indicator_type === 'nbp_reference_rate');
          const cpiIndicator = latestIndicators?.find((i: any) => i.indicator_type === 'cpi_estimate');

          const response = {
            lastUpdated: latestBonds?.[0]?.fetched_at || new Date().toISOString(),
            nbpReferenceRate: nbpIndicator?.value ?? 4.00,
            cpiEstimate: cpiIndicator?.value ?? 4.90,
...
async function scrapeBondData(): Promise<{
  ...
  try {
    ...
    console.log(`Phase 1: found ${detailUrls.size} bond detail URLs:`, Object.fromEntries(detailUrls));
    ...
    const bondMeta: Record<string, any> = {
      OTS: { name: '3-miesięczne oszczędnościowe', category: 'fixed', tenorMonths: 3,
             earlyRedeemPenaltyPct: 100, minHoldMonths: 1,
             description: 'Stałe oprocentowanie przez cały okres.',
             notes: 'Najkrótszy dostępny instrument. Idealne dla płynności.' },
      ROR: { name: '12-miesięczne (stopa NBP)', category: 'nbp_indexed', tenorMonths: 12,
             earlyRedeemFixedFee: 0.50, minHoldMonths: 1,
             notes: 'Odsetki wypłacane co miesiąc.' },
      DOR: { name: '2-letnie (stopa NBP)', category: 'nbp_indexed', tenorMonths: 24,
             earlyRedeemFixedFee: 0.70, minHoldMonths: 1,
             notes: 'Odsetki wypłacane co miesiąc. Lepsza marża niż ROR.' },
      TOS: { name: '3-letnie stałoprocentowe', category: 'fixed', tenorMonths: 36,
             earlyRedeemFixedFee: 1.00, minHoldMonths: 1,
             notes: 'Zysk znany z góry. Chroni przed obniżkami stóp NBP.' },
      COI: { name: '4-letnie (inflacja CPI)', category: 'cpi_indexed', tenorMonths: 48,
             earlyRedeemFixedFee: 2.00, minHoldMonths: 1,
             notes: 'Ochrona przed inflacją od roku 2. Brak kapitalizacji.' },
      EDO: { name: '10-letnie (inflacja CPI)', category: 'cpi_indexed', tenorMonths: 120,
             earlyRedeemFixedFee: 3.00, minHoldMonths: 1,
      ...
      [...detailUrls.entries()].map(async ([symbol, url]) => {
        try {
          ...
          if (meta.category === 'fixed') {
            ...
            const fullRateText = norm.match(/Oprocentowanie[\s:]+(.{10,300}?)(?:Kapitalizacja|Wypłata|Sprzedaż)/i)?.[1] ?? '';
            ...
            bondEntry.nbpMarginPct = margin ?? 0.00; // ROR has 0 margin

            const marginStr = margin && margin > 0 ? ` + ${margin.toFixed(2).replace('.', ',')}%` : '';
            ...
            const fullRateText = norm.match(/Oprocentowanie[\s:]+(.{10,400}?)(?:Kapitalizacja|Wypłata|Sprzedaż)/i)?.[1] ?? '';
            ...
            const cpiMarginMatch = fullRateText.match(/mar[żz]a\s+([0-9]+[,\.][0-9]+)\s*%/i)
            ...
            const marginStr = cpiMargin !== undefined ? ` + ${cpiMargin.toFixed(2).replace('.', ',')}%` : '';
    ...
    const nbpRate = 4.00; // Current NBP reference rate (May 2026)
    const cpiEstimate = 4.90; // Current CPI estimate (GUS April 2026)
...
async function storeBondData(scrapedData: {
  bonds: any[];
  nbpRate: number;
  cpiEstimate: number;
}, supabase: any) {
  ...
  try {
    ...
    const indicatorInserts = [
      { indicator_type: 'nbp_reference_rate', value: scrapedData.nbpRate, unit: '%', fetched_at: now, is_active: true, valid_from: now },
      { indicator_type: 'cpi_estimate', value: scrapedData.cpiEstimate, unit: '%', fetched_at: now, is_active: true, valid_from: now }
    ];
