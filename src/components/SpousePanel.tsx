  ...
  return (
    <div className="flex items-center justify-between gap-4 py-3 group">
      <div className="flex items-center gap-3">
        {Icon && <Icon className="w-4 h-4 text-muted-foreground group-hover:text-accent transition-colors" />}
        <div>
              ...
              {filteredMembers.map((member) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm rounded-xl hover:bg-accent-soft hover:text-accent transition-all ${
                    spouse.assignedUserId === member.user_id ? "bg-accent-soft text-accent font-bold" : "font-medium"
                  }`}
                >
                  {member.label}
                </button>
      ...
      <div className="p-6 sm:p-8 grid lg:grid-cols-2 gap-10">
        ...
        <div className="space-y-10">
              ...
              {spouse.inputs.companyCarEnabled && (
                <div className="grid gap-4 pt-2 animate-in slide-in-from-top-2 duration-300">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <Label className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Typ Wyceny</Label>
                      <Select value={spouse.inputs.companyCarMode} onValueChange={(v) => set("companyCarMode", v as SalaryInputs["companyCarMode"])}>
                        <SelectTrigger className="rounded-xl border-border/50 h-11"><SelectValue /></SelectTrigger>
                        <SelectContent className="rounded-xl"><SelectItem value="statutory">Ustawowy</SelectItem><SelectItem value="manual">Własny</SelectItem></SelectContent>
                      </Select>
                    </div>
                    {spouse.inputs.companyCarMode === "statutory" ? (
                      <div className="space-y-1.5">
                        <Label className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Moc / Napęd</Label>
                        <Select value={spouse.inputs.companyCarStatutoryValue} onValueChange={(v) => set("companyCarStatutoryValue", v as SalaryInputs["companyCarStatutoryValue"])}>
                          <SelectTrigger className="rounded-xl border-border/50 h-11"><SelectValue /></SelectTrigger>
                          <SelectContent className="rounded-xl"><SelectItem value="250">Do 60kW / EV</SelectItem><SelectItem value="400">Standard</SelectItem></SelectContent>
                        </Select>
                      </div>
                    ) : (
                      <NumberField label="Kwota Mies." value={spouse.inputs.companyCarManualAmount} onChange={(n) => set("companyCarManualAmount", n)} />
                    )}
                  </div>
                </div>
              )}
          ...
          <Collapsible open={showAdvanced} onOpenChange={setShowAdvanced} className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display text-sm font-bold uppercase tracking-widest text-muted-foreground">Parametry Podatkowe & PPK</h3>
              </div>
            ...
            <CollapsibleContent className="space-y-6 pt-2 animate-in slide-in-from-top-4 duration-300">
              ...
              <div className="grid grid-cols-2 gap-8 bg-background/40 p-6 rounded-[2rem] border border-border/50">
                 <div className="space-y-4">
                    <div className="flex justify-between items-end">
                      <Label className="text-[10px] font-bold uppercase tracking-widest">PPK Pracownik</Label>
                      <span className="text-xs font-mono font-bold text-accent">{spouse.inputs.ppkEmployeeRate}%</span>
                    </div>
                    <Slider value={[spouse.inputs.ppkEmployeeRate]} min={0} max={4} step={0.1} onValueChange={([v]) => set("ppkEmployeeRate", v)} />
                 </div>
                 <div className="space-y-4">
                    <div className="flex justify-between items-end">
                      <Label className="text-[10px] font-bold uppercase tracking-widest">PPK Pracodawca</Label>
                      <span className="text-xs font-mono font-bold text-accent">{spouse.inputs.ppkEmployerRate}%</span>
                    </div>
                    <Slider value={[spouse.inputs.ppkEmployerRate]} min={0} max={4} step={0.1} onValueChange={([v]) => set("ppkEmployerRate", v)} />
                 </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-[1.5rem] bg-background/40 border border-border/50">
                  <ToggleRow label="PIT-2" hint="Kwota wolna 300 zł/mc" checked={spouse.inputs.pit2} onChange={(v) => set("pit2", v)} />
                </div>
                <div className="p-4 rounded-[1.5rem] bg-background/40 border border-border/50">
                  <ToggleRow label="Ulga <26 lat" checked={spouse.inputs.age26Exempt} onChange={(v) => set("age26Exempt", v)} />
                </div>
                  ...
                  {spouse.inputs.autorskiSharePct > 0 && (
                    <div className="pt-4 space-y-3">
                       <div className="flex justify-between">
                          <span className="text-[10px] font-bold uppercase text-muted-foreground">Udział w pensji</span>
                          <span className="text-xs font-bold text-accent">{spouse.inputs.autorskiSharePct}%</span>
                       </div>
                       <Slider value={[spouse.inputs.autorskiSharePct]} min={0} max={100} step={5} onValueChange={([v]) => set("autorskiSharePct", v)} />
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="bg-success/5 rounded-[2.5rem] p-8 border border-success/20 space-y-6 relative overflow-hidden">
            ...
            <SectionHeader icon={TrendingUp} title="Premia & Bonusy" desc="Dodatkowe wpływy roczne lub kwartalne." />
            ...
            {spouse.inputs.bonusMonth > 0 && (
              <div className="grid gap-6 pt-4 border-t border-success/10 animate-in slide-in-from-bottom-2 duration-300">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Miesiąc wypłaty</Label>
                    <Select value={String(spouse.inputs.bonusMonth)} onValueChange={(v) => set("bonusMonth", parseInt(v))}>
                      <SelectTrigger className="rounded-xl border-success/20 h-11 bg-background/50"><SelectValue /></SelectTrigger>
                  ...
                  <NumberField label="Wartość % Rocznie" value={spouse.inputs.bonusPct} onChange={(v) => set("bonusPct", v)} suffix="%" />
                </div>
                
                <div className="bg-success-soft/30 p-4 rounded-2xl border border-success/10 flex items-center justify-between">
                   <div className="space-y-1">
                      <p className="text-xs font-bold text-success-foreground">Obliczaj automatycznie?</p>
                      <p className="text-[10px] text-muted-foreground font-medium">Na podstawie % i rocznej pensji</p>
                   </div>
                   <Switch checked={spouse.inputs.bonusOverrideGross === null} onCheckedChange={(v) => set("bonusOverrideGross", v ? null : (spouse.inputs.gross * 12 * (spouse.inputs.bonusPct / 100)))} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="sticky top-8 space-y-6">
            ...
            <div className="relative overflow-hidden rounded-[2.5rem] bg-foreground p-8 shadow-2xl group animate-in zoom-in-95 duration-700">
               ...
               <div className="relative z-10 grid grid-cols-2 gap-4 mt-12 pt-8 border-t border-background/10">
                  <div>
                    <p className="text-[10px] font-bold text-background/40 uppercase tracking-widest mb-1">Brutto</p>
                    <p className="text-background font-mono font-bold">{formatPLN2(r.gross)}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-bold text-background/40 uppercase tracking-widest mb-1">Koszt Pracodawcy</p>
                    <p className="text-background font-mono font-bold">{formatPLN2(r.totalEmployerCost)}</p>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-card rounded-[2.5rem] border border-border p-8 shadow-warm">
               ...
               <div className="space-y-1">
                  ...
                  {r.companyCarTaxable > 0 && <Row label="Użytek auta (przychód)" value={r.companyCarTaxable} muted />}
                  <div className="h-4" />
                  <Row label={<div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-destructive" /> ZUS (Emeryt/Rent/Chor)</div>} value={-r.zusTotal} negative />
                  <Row label={<div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-amber-500" /> Zdrowotna 9%</div>} value={-r.health} negative />
                  {r.ppkEmployee > 0 && <Row label={<div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-blue-500" /> PPK Pracownik</div>} value={-r.ppkEmployee} negative />}
                  <Row label={<div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-accent" /> Zaliczka na PIT</div>} value={-r.pit} negative />
                  <div className="h-4" />
                  ...
                  <div className="flex items-center justify-between">
                     ...
                     <span className="font-display text-2xl font-bold text-accent">{formatPLN2(r.net)}</span>
                  </div>
               </div>
            </div>
            ...
            {!spouse.inputs.outsideFirstThreshold && monthsToSecondThreshold && (
              <div className="bg-warning-soft/30 border border-warning/20 rounded-[2rem] p-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10"><Zap className="w-12 h-12 text-warning" /></div>
                <div className="relative z-10">
                   <p className="text-[10px] font-bold uppercase tracking-widest text-warning-foreground mb-2">Drugi Próg Podatkowy (32%)</p>
                   <p className="text-sm font-semibold leading-relaxed">
                     Przekroczysz limit 120k w <span className="text-warning-foreground underline decoration-warning/30 underline-offset-4">{monthsToSecondThreshold >= 12 ? "grudniu" : monthIndexToName(monthsToSecondThreshold)}</span>.
                   </p>
...
function monthIndexToName(month: number): string {
  const names = ["styczniu", "lutym", "marcu", "kwietniu", "maju", "czerwcu", "lipcu", "sierpniu", "wrześniu", "październiku", "listopadzie", "grudniu"];
