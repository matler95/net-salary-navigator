  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-80px)] bg-background">
      ...
      <div className={cn(
        ...
        cashflowPositive ? "bg-success/10 border-success/20" : "bg-destructive/10 border-destructive/20"
      )}>
        <div>
          <p className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">CF / Miesiąc</p>
          <p className={cn("font-display text-2xl leading-none", cashflowPositive ? "text-success" : "text-destructive")}>
            {cashflowPositive ? "+" : ""}{formatPLN2(r.monthlyCashflow)}
          </p>
        </div>
        <div className="text-right">
          <p className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">Min. czynsz</p>
      ...
      <div className="h-1 bg-muted/30 w-full relative">
        <div className="absolute top-0 left-0 h-full bg-accent transition-all duration-300" style={{ width: `${(step / 4) * 100}%` }} />
      </div>
