...
export function ScenarioHeader() {
  const { r, minRent, verdict, rentMargin, rentMarginPct, cashflowPositive, s, steadyCashflow } =
    useRealEstate();
  ...
  return (
    <div className="sticky top-0 z-40 bg-background/80 backdrop-blur-xl border-b border-border/40 pb-4 pt-2 mb-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div
          className={cn(
            "hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-bold shadow-sm",
            vm.color,
          )}
        >
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4">
        ...
        <div
          className={cn(
            ...
            cashflowPositive
              ? "bg-success/10 border-success/30 shadow-warm"
        ...
        >
            ...
            <TooltipProvider delayDuration={150}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    type="button"
                    className="cursor-help text-muted-foreground/50 hover:text-muted-foreground transition-colors focus:outline-none"
                  >
                ...
                <TooltipContent side="bottom" className="max-w-[220px] text-center leading-relaxed">
                  ...
                  <p>
                    Miesięczny zysk przy normalnym, pełnym wynajmie — bez uwzględniania pustostanu.
                  </p>
                  {vacancyMonths > 0 && (
                    <p className="mt-1 opacity-80">
                      Rok&nbsp;1 (z {vacancyMonths}&nbsp;m-c pustostanu) pokazany niżej jako wartość
                      pomocnicza.
                    </p>
                  )}
                </TooltipContent>
          ...
          <p
            className={cn(
              "font-display text-xl sm:text-3xl tracking-tight leading-none",
          ...
          {vacancyMonths > 0 && (
            <p
              className={cn(
                ...
                r.monthlyCashflow >= 0 ? "text-success/70" : "text-warning-foreground",
              )}
            >
              Rok 1 śr: {r.monthlyCashflow >= 0 ? "+" : ""}
        ...
        <div className="rounded-2xl border border-border bg-card p-3 sm:p-4 shadow-sm flex flex-col justify-center">
          ...
          <p className="font-display text-xl sm:text-3xl tracking-tight leading-none">
            {r.irrAnnualPct.toFixed(1)}%
          </p>
        </div>
            ...
            <TooltipProvider delayDuration={150}>
              <Tooltip>
                ...
                <TooltipContent side="bottom" className="max-w-[220px] text-center leading-relaxed">
                  ...
                  <p>
                    Czynsz pokrywający ratę kredytu, koszty stałe i podatek przy pełnym wynajmie.
                  </p>
                  {(s.renovationMonths || 0) + (s.tenantSearchMonths || 0) > 0 && (
                    <p className="mt-1 opacity-80">
                      Pustostan roku&nbsp;1 wpływa na <em>Zysk miesięczny</em>, nie na ten próg.
                    </p>
                  )}
                </TooltipContent>
              </Tooltip>
      ...
      <div className="px-1">
        <div className="flex justify-between items-baseline mb-1.5">
          ...
          <span
            className={cn("text-xs font-bold", isRentHealthy ? "text-success" : "text-destructive")}
          >
            Twój czynsz: {formatPLN(s.monthlyRent)} ({rentMarginPct >= 0 ? "+" : ""}
            {rentMarginPct.toFixed(0)}%)
          </span>
        </div>
        <div className="h-2 bg-muted/40 rounded-full overflow-hidden relative border border-border/50">
          <div
            className={cn(
              ...
              isRentHealthy ? "bg-success" : "bg-destructive",
            )}
