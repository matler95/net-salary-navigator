...
import {
  ...
  amortizationScheduleDecreasing,
  ...
  return (
    <Tabs defaultValue="flow" className="space-y-6">
      <TabsList className="w-full flex sm:w-auto overflow-x-auto bg-transparent p-0 border-b border-border/40 rounded-none h-auto justify-start">
        <TabsTrigger
          value="flow"
          className="rounded-none border-b-2 border-transparent data-[state=active]:border-accent data-[state=active]:bg-transparent data-[state=active]:shadow-none px-4 py-2 bg-transparent"
        >
          Przepływ Pieniędzy
        </TabsTrigger>
        <TabsTrigger
        ...
        <TabsTrigger
          value="obligacje"
          className="rounded-none border-b-2 border-transparent data-[state=active]:border-accent data-[state=active]:bg-transparent data-[state=active]:shadow-none px-4 py-2 bg-transparent whitespace-nowrap"
        >
          vs. Obligacje
        </TabsTrigger>
      </TabsList>
...
function CashflowTab() {
  ...
  const waterfallSteps: {
    ...
    {
      label: "Koszty stałe właściciela",
    ...
    ...(effectiveOverpayment > 0
      ...
      : []),
    {
      label: `Podatek ryczałtowy (${s.taxRatePct}% od czynszu)`,
      value: -monthlyTax,
      tone: "warning",
    },
  ];
      ...
      <div className="bg-card rounded-3xl p-6 sm:p-8 border border-border shadow-sm space-y-6">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Zysk przy <span className="font-semibold text-foreground">pełnym wynajmie</span> — jeden
            normalny miesiąc bez pustostanu.
            {hasVacancy && (
              <>
                {" "}
                Rok&nbsp;1 z {vacancyMonths}&nbsp;m-cami pustostanu:{" "}
                <span
                  className={cn(
                    "font-semibold",
                ...
                >
                  {formatPLN2(r.monthlyCashflow)}/m-c średnio
                </span>{" "}
                (czynsz z {12 - vacancyMonths} m-cy rozłożony na 12).
              </>
            )}
          </p>
        </div>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isSub && "pt-2 pb-1 border-y border-border/50",
                )}
              >
                  ...
                  <div>
                    <span
                      className={cn(
                        ...
                        isSub ? "text-foreground" : "text-muted-foreground",
                      )}
                    >
                      {step.label}
                    </span>
                ...
                {step.subs && (
                  <div className="pl-4 space-y-1 mt-1 border-l-2 border-border ml-1">
                    {step.subs.map((sub, j) => (
                      <div
                        key={j}
                        className="flex justify-between items-baseline text-xs text-muted-foreground opacity-70"
                      >
                        <span>{sub.label}</span>
              ...
              <div className="mt-3 flex items-center gap-2 p-3 bg-warning/5 rounded-xl border border-warning/20">
                ...
                <p className="text-xs text-muted-foreground">
                  <span className="font-semibold text-foreground">
                    Rok 1 (z {vacancyMonths} m-c pustostanu):{" "}
                  </span>
                  <span
                    className={cn(
                      "font-semibold",
                  ...
                  >
                    {formatPLN2(r.monthlyCashflow)}/m-c
                  </span>{" "}
                  — czynsz z {12 - vacancyMonths} m-cy podzielony na 12. Różnica:{" "}
                  <span className="font-mono">
                    {formatPLN2(steadyCashflow - r.monthlyCashflow)}
                  </span>
                  .
                </p>
              </div>
        ...
        <div className="bg-card p-5 rounded-2xl border border-border shadow-sm">
          ...
          <p
            className={cn(
              ...
              rentMargin >= 0 ? "text-success" : "text-destructive",
            )}
          >
            Zapas: {rentMargin >= 0 ? "+" : ""}
            {formatPLN(rentMargin)} ({rentMarginPct.toFixed(0)}%)
          </p>
          <p className="text-[10px] text-muted-foreground mt-2 leading-relaxed">
            Czynsz pokrywający ratę, koszty i podatek przy pełnym wynajmie. Nie uwzględnia
            pustostanu — patrz Zysk rok 1.
          </p>
        </div>
      </div>
    </div>
        ...
        <div className="h-64 sm:h-80 mb-6">
          <ResponsiveContainer>
            <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
              ...
              />
              <Tooltip
                formatter={(v: number) => formatPLN(Math.abs(v))}
              ...
              <Area
                type="monotone"
                dataKey="negativeLoan"
                name="Pozostały kredyt"
        ...
        <div className="space-y-3 bg-muted/20 p-4 rounded-2xl border border-border/50">
          ...
          {groupedMilestones.map((m, i) => (
            <div key={i} className="flex items-start gap-3 text-sm">
              <div className="bg-background border shadow-sm px-2 py-0.5 rounded-md font-mono text-xs font-bold text-accent shrink-0">
                Rok {m.year}
              </div>
...
function FlowTab() {
  ...
  const totalBaselineMortgagePayments =
    r.totalOperationalCosts - totalTax - totalMonthlyCosts - totalInsurance;
    ...
    <div className="space-y-6">
      <div className="bg-card rounded-3xl p-6 sm:p-8 border border-border shadow-sm">
        ...
        <p className="text-xs text-muted-foreground mb-6">
          Pełny obraz inwestycji: ile włożyłeś, ile wróciło co miesiąc, ile dostaniesz przy
          sprzedaży.
        </p>
        ...
        <div className="space-y-4">
                ...
                <div className="flex justify-between border-t border-border/50 pt-2 mt-2">
                  <div>
                    ...
                    <p className="text-[10px] italic opacity-70 mt-0.5">
                      Łączna kwota miesięcy, gdy koszty &gt; czynsz — już uwzględniona w cashflow
                      poniżej
                    </p>
                  </div>
                  <span className={`font-mono ${valueToneClass(-totalCumNegative)}`}>
                    {formatSignedPLN(-totalCumNegative)}
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="border border-border rounded-3xl bg-card p-5 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-1">
                  Cashflow przez {s.holdingYears} lat
                </p>
                <h4 className="font-semibold text-base">Co wróciło co miesiąc</h4>
              </div>
              <div className={`font-mono font-semibold ${valueToneClass(r.totalCashflow)}`}>
            ...
            <div className="mt-4 space-y-2 text-sm text-muted-foreground">
              <div className="flex justify-between">
                ...
                <span className={`font-mono ${valueToneClass(totalGrossRent)}`}>
                  {formatSignedPLN(totalGrossRent)}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className={`font-mono ${valueToneClass(-totalInsurance)}`}>
                  {formatSignedPLN(-totalInsurance)}
                </span>
              </div>
              <div className="flex justify-between">
                <span>Minus: koszty stałe ({s.monthlyCosts.toFixed(0)} zł/m-c)</span>
                <span className={`font-mono ${valueToneClass(-totalMonthlyCosts)}`}>
                  {formatSignedPLN(-totalMonthlyCosts)}
                </span>
              </div>
              <div className="flex justify-between">
                <span>Minus: podatek ryczałtowy ({s.taxRatePct}%)</span>
                <span className={`font-mono ${valueToneClass(-totalTax)}`}>
                  {formatSignedPLN(-totalTax)}
                </span>
              </div>
                ...
                <div className="pt-2 border-t border-border/50 space-y-1">
                  <div className="flex justify-between">
                    <div>
                      ...
                      <p className="text-[10px] italic opacity-70 mt-0.5">
                        Realny wydatek — budujesz kapitał szybciej
                      </p>
                    </div>
                    <span className={`font-mono ${valueToneClass(-r.totalOverpaymentPaid)}`}>
                      {formatSignedPLN(-r.totalOverpaymentPaid)}
                    </span>
                  </div>
            ...
            <div className="mt-3 p-3 bg-muted/30 rounded-xl text-[10px] text-muted-foreground leading-relaxed">
              <span className="font-semibold">Jak sprawdzić:</span> czynsz (
              {formatPLN(totalGrossRent)}) − raty ({formatPLN(totalBaselineMortgagePayments)}) −
              ubezpieczenie ({formatPLN(totalInsurance)}) − koszty ({formatPLN(totalMonthlyCosts)})
              − podatek ({formatPLN(totalTax)})
              {r.totalOverpaymentPaid > 0
                ? ` − nadpłaty (${formatPLN(r.totalOverpaymentPaid)}) + odsetki zaoszcz. (${formatPLN(r.totalOverpaymentPaid - r.netOverpaymentCost)})`
                : ""}{" "}
              ={" "}
              <span
                className={cn(
                  "font-bold",
                  r.totalCashflow >= 0 ? "text-success" : "text-destructive",
                )}
              >
                {formatSignedPLN(r.totalCashflow)}
              </span>
          ...
          {s.sellAtEnd && (
            <div className="border border-border rounded-3xl bg-card p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-1">
                    Sprzedaż po {s.holdingYears} latach
                  </p>
                  <h4 className="font-semibold text-base">Co dostaniesz na wyjściu</h4>
                </div>
                <div className={`font-mono font-semibold ${valueToneClass(netFromSale)}`}>
                  {formatSignedPLN(netFromSale)}
                </div>
              </div>
              <div className="mt-4 space-y-2 text-sm text-muted-foreground">
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-mono ${valueToneClass(r.yearly[r.yearly.length - 1].propertyValue)}`}
                  >
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-mono ${valueToneClass(-remainingLoan)}`}>
                    {formatSignedPLN(-remainingLoan)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Minus: koszty transakcji (~2%)</span>
                  <span className={`font-mono ${valueToneClass(-saleCosts)}`}>
                    {formatSignedPLN(-saleCosts)}
                  </span>
                </div>
                <div className="flex justify-between font-semibold border-t border-border pt-3">
                  <span>Zysk ze sprzedaży (netto)</span>
                  <span className={`font-mono ${valueToneClass(netFromSale)}`}>
                    {formatSignedPLN(netFromSale)}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-[1.6fr_1fr]">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={
                        s.overpaymentMonthly !== null
                          ? formatLocaleAmount(s.overpaymentMonthly)
                          : ""
                      }
                      onChange={(e) => {
                  ...
                  <div className="rounded-2xl border border-border bg-muted/20 p-4">
                    ...
                    <p className="font-display text-2xl mt-2">
                      {formatPLN(s.overpaymentMonthly ?? requiredOverpayment)}
                    </p>
                  </div>
                </div>
                ...
                {s.overpaymentMonthly !== null && s.overpaymentMonthly < requiredOverpayment && (
                  <div className="rounded-2xl border border-warning/50 bg-warning/10 p-3 text-sm text-warning-foreground">
                    Kredyt nie zostanie spłacony w czasie analizy. Pozostała kwota:{" "}
                    <span className="font-semibold">{formatPLN(remainingLoan)}</span>
                  </div>
                )}
                    ...
                    <div className="grid grid-cols-3 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-mono font-semibold text-muted-foreground">
                          {formatPLN(r.netOverpaymentCost)}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-card border border-border rounded-3xl p-5 shadow-sm">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="border-t border-border pt-3 flex justify-between font-bold text-lg">
                ...
                >
                  {finalProfit >= 0 ? "+" : ""}
                  {formatPLN(finalProfit)} ({finalProfitPct.toFixed(1)}%)
                </span>
              </div>
            ...
            <div className="mt-4 p-3 bg-muted/20 rounded-xl text-[10px] text-muted-foreground leading-relaxed">
              ...
              {s.sellAtEnd ? ` + sprzedaż (${formatSignedPLN(netFromSale)})` : ""} − wkład własny (
              {formatPLN(r.totalUpfront)}) ={" "}
              <span
                className={cn("font-bold", finalProfit >= 0 ? "text-success" : "text-destructive")}
              >
                {formatSignedPLN(finalProfit)}
              </span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-card rounded-3xl p-6 sm:p-8 border border-border shadow-sm overflow-x-auto">
        <div className="mb-6">
          <h3 className="font-display text-xl mb-1 flex items-center gap-2">Analiza "Co jeśli?"</h3>
          <p className="text-xs text-muted-foreground">
            Jak zmiana stóp procentowych i czynszu wpłynie na miesięczny zysk przy pełnym wynajmie.
          </p>
        </div>
            ...
            <thead>
              <tr>
                <th
                  rowSpan={2}
                  colSpan={2}
                  className="border-b-2 border-r-2 border-border/50 bg-muted/20"
                ></th>
                <th
                  colSpan={5}
                  className="py-2 text-[10px] uppercase tracking-widest text-muted-foreground border-b border-border/50 bg-muted/10"
                >
                  Czynsz brutto
                </th>
              </tr>
              <tr>
                {safeRentSteps.map((rent, i) => (
                  <th
                    key={i}
                    className="py-2 font-mono text-sm border-b-2 border-border/50 bg-muted/10 font-bold"
                  >
                    {formatPLN(rent)}
                  </th>
                ))}
              ...
              {safeRateSteps.map((rate, rIdx) => (
                <tr key={rIdx}>
                  {rIdx === 0 && (
                    <th
                      rowSpan={5}
                      className="px-2 text-[10px] uppercase tracking-widest text-muted-foreground border-r border-border/50 [writing-mode:vertical-lr] rotate-180 bg-muted/10"
                    >
                      Oprocentowanie
                    </th>
                  )}
                  <th className="py-3 px-3 text-right font-mono text-sm border-r-2 border-border/50 bg-muted/10 font-bold">
                    {rate.toFixed(1)}%
                  </th>
                      ...
                      <td key={cIdx} className="p-1">
                        <div
                          className={cn(
                            ...
                            tone,
                            isCurrentCell(cIdx, rIdx) &&
                              "ring-2 ring-foreground shadow-sm bg-background border-none",
                          )}
                        >
                          {cf > 0 ? "+" : ""}
                          {cf.toFixed(0)} zł
      ...
      <div className="flex justify-between items-baseline">
        ...
        <span
          className={cn(
            ...
            tone === "destructive" ? "text-destructive" : "text-warning-foreground",
          )}
        >
          {level}
        </span>
...
function BudgetTab() {
  ...
  const dtiBefore =
    budgetImpact.totalNetIncome > 0
      ? ((budgetImpact.totalNetIncome - budgetImpact.currentDisposable) /
          budgetImpact.totalNetIncome) *
        100
      : 0;
    ...
    <div className="space-y-6">
      <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between border-b border-border/50 pb-2">
              ...
              <span className="font-mono text-sm">
                -{formatPLN(budgetImpact.totalNetIncome - budgetImpact.currentDisposable)}
              </span>
            </div>
            <div className="flex justify-between pt-2">
              <span className="font-bold">Nadwyżka w portfelu</span>
              <span className="font-display text-xl">
                {formatPLN(budgetImpact.currentDisposable)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-accent/5 to-accent/10 rounded-3xl p-6 sm:p-8 border border-accent/20 shadow-sm relative overflow-hidden">
          <p className="text-[10px] uppercase tracking-[0.2em] text-accent font-bold mb-4">
            Po Inwestycji
          </p>
          <div className="space-y-3 relative z-10">
            <div className="flex justify-between border-b border-accent/20 pb-2">
              ...
              <span className="font-mono text-sm text-success">
                {formatPLN(budgetImpact.totalNetIncome + s.monthlyRent)}
              </span>
            </div>
            <div className="flex justify-between border-b border-accent/20 pb-2">
              ...
              <span className="font-mono text-sm text-destructive">
                ...
                {formatPLN(
                  budgetImpact.totalNetIncome -
                    budgetImpact.currentDisposable +
                    r.monthlyPmt +
                    s.monthlyCosts +
                    r.monthlyTax +
                    s.mortgageInsuranceMonthly,
            ...
            <div className="flex justify-between pt-2">
              ...
              <div className="text-right">
                ...
                >
                  {r.monthlyCashflow >= 0 ? "+" : ""}
                  {formatPLN(r.monthlyCashflow)} zysku z najmu
                </p>
              </div>
            </div>
          </div>
      ...
      >
        ...
        <div>
          ...
          <p className="font-mono text-lg font-bold mt-1">
            {dtiBefore.toFixed(0)}% <span className="text-muted-foreground mx-2">→</span>{" "}
            {dtiAfter.toFixed(0)}%
          </p>
