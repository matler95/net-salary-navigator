...
import { Home, Building2, Wallet, TrendingUp } from "lucide-react";
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      <div className="space-y-6">
        ...
        <section data-section="1" className="bg-card rounded-3xl p-6 border border-border shadow-sm space-y-5">
          ...
          <div className="space-y-4">
            <NumField
              ...
              feedback={
                <>
                  ...
                  <span className="text-destructive font-bold">+10k zł = {getCfDelta({ purchasePrice: s.purchasePrice + 10000 })}</span> */}
                </>
              }
            />
            <SliderField
              ...
              feedback={
                <>
                  ...
                  {/* <span className="opacity-50">|</span> */}
                  {/* <span className="text-success font-bold">+5% = {getCfDelta({ downPaymentPct: s.downPaymentPct + 5 })}</span> */}
                </>
              }
            />
            <NumField
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  <div className={cn("w-3 h-3 rounded-sm border", s.hasAgency ? "bg-accent border-accent" : "border-muted-foreground")} />
                  Kupuję z agencją nieruchomości (+2%)
                </button> */}
        ...
        {/* Sekcja 2: Kredyt */}
        <section data-section="2" className="bg-card rounded-3xl p-6 border border-border shadow-sm space-y-5">
          <div className="flex items-center gap-3 border-b border-border/50 pb-4">
            <div className="w-10 h-10 rounded-xl bg-accent/10 text-accent flex items-center justify-center shrink-0">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2 p-1 bg-muted/30 rounded-xl">
              ...
              >
                Raty malejące
                <div className="flex gap-[1px] h-3 items-end opacity-50">
                  <div className="w-1.5 h-3 bg-current" /><div className="w-1.5 h-2.5 bg-current" /><div className="w-1.5 h-2 bg-current" /><div className="w-1.5 h-1.5 bg-current" />
                </div>
              </button>
            </div>
            ...
            {/* <div className="space-y-1">
              <SliderField
                ...
                feedback={<span className="text-destructive font-bold">+1% stopy = {getCfDelta({ mortgageRatePct: s.mortgageRatePct + 1 })}</span>}
              />
              <div className="flex gap-2 justify-end">
                <button onPointerDown={() => updateS({ mortgageRatePct: s.mortgageRatePct + 0.5 })} className="text-[9px] px-2 py-0.5 rounded border hover:bg-muted font-bold">+0.5%</button>
                <button onPointerDown={() => updateS({ mortgageRatePct: s.mortgageRatePct + 1 })} className="text-[9px] px-2 py-0.5 rounded border hover:bg-muted font-bold">+1.0%</button>
                <button onPointerDown={() => updateS({ mortgageRatePct: s.mortgageRatePct + 2 })} className="text-[9px] px-2 py-0.5 rounded border hover:bg-muted font-bold">+2.0%</button>
              </div>
            </div> */}
      ...
      <div className="space-y-6">
        {/* Sekcja 3: Wynajem */}
        <section data-section="3" className="bg-card rounded-3xl p-6 border border-border shadow-sm space-y-5">
          <div className="flex items-center gap-3 border-b border-border/50 pb-4">
            <div className="w-10 h-10 rounded-xl bg-accent/10 text-accent flex items-center justify-center shrink-0">
          ...
          <div className="space-y-4">
            ...
            // feedback={<span className="text-success font-bold">+100 zł = {getCfDelta({ monthlyRent: s.monthlyRent + 100 })}</span>}
            />
        ...
        {/* Sekcja 4: Prognozy */}
        <section data-section="4" className="bg-card rounded-3xl p-6 border border-border shadow-sm space-y-5">
          <div className="flex items-center gap-3 border-b border-border/50 pb-4">
            <div className="w-10 h-10 rounded-xl bg-accent/10 text-accent flex items-center justify-center shrink-0">
